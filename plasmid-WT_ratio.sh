# A wrapper script for analyzing plasmid-WT ratio at different user-defined cut-off criteria
...
for BC3_min_cutoff in $BC3_min; do
    ...
    outdir="$params_dir_out_1/tables_consensus.BC3cutoff$BC3_min_cutoff.plasmid-WT_ratio"
    if [ ! -d $outdir ]; then 
            ...
            for sam_BX_tag in ''; do 
                ...
                if [ ! -z "$table_in1_dir" ] && [ ! -z "$BC3_min_cutoff" ] && [ ! -z "$outdir" ] && [ ! -z "$log_out1" ]; then
                    echo 'All files and parameters are found'
                     ...
                     # Run jobs to analyze plasmid-WT ratios
                    if [ $1 -eq 1 ]; then
                       sleep 1
                            
                       sbatch -o "$log_out1.out" -e "$log_out1.err" --mem=28000 \
                              -p hive1d,hive7d,hiveunlim,queen \
                              --wrap "module load R/3.6.0-foss-2019a; Rscript plasmid-WT_ratio_v2.R $table_in1_dir $outdir \"$plasmid\""
