# A wrapper script for creating plots depicting substitution counts per read position for different cutoff criteria
...
for BC3_min_cutoff in $BC3_min; do
    ...
    outdir="$params_dir_out_1/tables_consensus.BC3cutoff$BC3_min_cutoff.plots"
    if [ ! -d $outdir ]; then 
    ...
    for i in ${!title[@]}; do
        ...
        for r1 in $refs1; do
            ...
            for sam_BX_tag in ''; do 
                ...
                log_out1="$logdir/$bam_name.""$tag1""plots"
                ...
                if [ ! -z "$table_in1_dir" ] && [ ! -z "$BC3_min_cutoff" ] && [ ! -z "$outdir" ] && [ ! -z "$log_out1" ]; then
                    ...
                    # Run jobs to create substitution count plots
                    if [ $1 -eq 1 ]; then
                       sleep 1
                            
                       sbatch -o "$log_out1.out" -e "$log_out1.err" --mem=28000 \
                              -p hive1d,hive7d,hiveunlim,queen \
                              --wrap "module load R/3.6.0-foss-2019a; Rscript plot_subs.R $r1 $table_in1_dir $params_3 $outdir $excl_pos $excl_sub $excl_var"
