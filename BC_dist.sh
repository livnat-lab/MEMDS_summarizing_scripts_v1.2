# A wrapper script for reporting size distribution of read families and secondary barcode distribution across same size families
    ...
    for i in ${!title[@]}; do
        ...
        for r1 in $refs1; do
            ...
            for sam_BX_tag in ''; do 
                ...
                table_out1="$outdir/$bam_name.""$tag1""BC_dist.txt"
                log_out1="$logdir/$bam_name.""$tag1""BC_log"
                ...
                if [ -f "$table_in1" ] && [ -d "$params_dir_out_1/tables.BC3cutoff$BC3_min_cutoff" ]; then
                    if [ ! -z "$table_in1" ] && [ ! -z "$table_out1" ] && [ ! -z "$BC3_min_cutoff" ]; then
                        ...
                        # Run jobs to analyze size and secondary barcode distribution in read families
                        if [ $1 -eq 1 ]; then
                            sleep 1

                            sbatch -o "$log_out1.out" -e "$log_out1.err" \
                                   -p hive1d,hive7d,hiveunlim,queen \
                                   --wrap "module load R/3.6.0-foss-2019a; Rscript BC_dist.R $table_in1 $table_out1"
