# A wrapper script for reporting relative proportion of BC3 groups with more than one read in them across read families
...
for BC3_min_cutoff in $BC3_min; do
    ...
    outdir="$params_dir_out_1/tables.BC3cutoff$BC3_min_cutoff.BC3_above1"
    if [ ! -d $outdir ]; then 
    ...
    for i in ${!title[@]}; do
        ...
        for r1 in $refs1; do
            ...
            # Delete three lines below to analyze "others" files. By default they are skipped, since they usually do not contain enough data for meaningful analysis
            if [[ $r2 == *"others"* ]]; then 
                continue
            fi                 
            ...
            for sam_BX_tag in ''; do
                ...
                table_out1="$outdir/$bam_name.""$tag1""BC3_above1.txt"
                log_out1="$logdir/$bam_name.""$tag1""BC3_above"
                ...
                if [ -f "$table_in1" ] && [ -d "$params_dir_out_1/tables.BC3cutoff$BC3_min_cutoff" ]; then
                    if [ ! -z "$table_in1" ] && [ ! -z "$table_out1" ] && [ ! -z "$BC3_min_cutoff" ]; then
                        ...
                        # Run jobs to analyze proportion of BC3 groups having more than one read in them across read families
                        if [ $1 -eq 1 ]; then
                            sleep 1

                            sbatch -o "$log_out1.out" -e "$log_out1.err" \
                                   -p hive1d,hive7d,hiveunlim,queen \
                                   --wrap "module load R/3.6.0-foss-2019a; Rscript BC3_size_above1.R $table_in1 $table_out1"
                        fi
