# A wrapper script for re-calculating error and mutation rates based on user-defined criteria
...
for BC3_min_cutoff in $BC3_min; do
    ...
    outdir="$params_dir_out_1/tables_consensus.BC3cutoff$BC3_min_cutoff.re-calc_err"
    if [ ! -d $outdir ]; then 
    ...
    for i in ${!title[@]}; do
        ...
        for r1 in $refs1; do
            ...
            for sam_BX_tag in ''; do 
                # Define BC3 tag variable added to the output file names
                ...
                log_out1="$logdir/$bam_name.""$tag1""re-err"
                ...
                if [ ! -z "$table_in1_dir" ] && [ ! -z "$BC3_min_cutoff" ] && [ ! -z "$outdir" ] && [ ! -z "$log_out1" ]; then
                    echo 'All parameters are found'
                    
                    # Run jobs to re-calculate error and mutation rates
                    if [ $1 -eq 1 ]; then
                       sleep 1
                            
                       sbatch -o "$log_out1.out" -e "$log_out1.err" --mem=28000 \
                              -p hive1d,hive7d,hiveunlim,queen \
                              --wrap "module load R/3.6.0-foss-2019a; Rscript re_calc_error_rate.R $r1 $table_in1_dir $params_3 $outdir $excl_pos $excl_sub $excl_var"
