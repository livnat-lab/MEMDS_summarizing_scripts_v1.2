...
for BC3_min_cutoff in $BC3_min; do
    ...
    outdir="$params_dir_out_1/tables.BC3cutoff$BC3_min_cutoff.BC5_nucl"
    if [ ! -d $outdir ]; then 
    ...
    for i in ${!title[@]}; do
        ...
        for r1 in $refs1; do
            ...
            for sam_BX_tag in ''; do
                ...
                table_out1="$outdir/$bam_name.""$tag1""BC5_nucl.txt"
                log_out1="$logdir/$bam_name.""$tag1""BC5_log"
                ...
                if [ -f "$table_in1" ] && [ -d "$params_dir_out_1/tables.BC3cutoff$BC3_min_cutoff" ]; then
                    if [ ! -z "$log_out1" ] && [ ! -z "$outdir" ] && [ ! -z "$table_out1" ] && [ ! -z "$BC3_min_cutoff" ]; then
                        ...
                        # Run jobs to analyze nucleotide distribution within primary barcodes
                        if [ $1 -eq 1 ]; then
                            sleep 1

                            sbatch -o "$log_out1.out" -e "$log_out1.err" \
                                   -p hive1d,hive7d,hiveunlim,queen \
                                   --wrap "module load R/3.6.0-foss-2019a; Rscript BC5_nucleotide_dist.R \"$table_in1\" \"$table_out1\" \"$size_f1\" \"$read_seq\" \"$read_pos\" \"$id_len\""
